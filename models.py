...
def build_CNN(input_shape, filters= [3], kernels = [4], activation = 'relu', optimizer= 'Adam'):
    ...
    optimizer = SGD(lr=0.5, decay=.01) if optimizer =='sgd' else optimizer = Adam(learning_rate=0.001)
...
def train_nn(model,X_train, y_train, X_test, y_test,  batch_size, epochs):
